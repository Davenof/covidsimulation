...
The DAYS monitor displays the number of days / assumed or projected number of days it takes, or will take for a given number of susceptible population to be infected. As mentioned earlier, each tick indicates one day in the model’s time scale.
...
When more than one person is infected from the start of the simulation, transmission occurs faster. Therefore, the more the number of infected persons at the onset, the faster the spread. 
...
The number of PROTECTED persons never decreases, and their colour never turns orange. That depicts and corroborates the claims that following recommended precautions to the letter could alter the rate of transmission. 

Persons could move around the entire length and breadth of the WORLD as they simulate what is possible in a small community, be it a formal or an informal community. The simulation models a setting where it is possible for one person to walk the entire perimeter of the community.
 
When no vulnerable person with the colour grey is left in the WORLD, the process stops automatically.


## THINGS TO TRY

The model simulates transmission and prevention of the coronavirus. Hence, try manipulating the slider values to achieve a near real-life coronavirus situation — taking strict abidance and non-adherence to recommended preventive measures into cognisance. 
...
The model could be extended to include a limitation to the extent persons could move around the community, as everyone for some reasons – medical, cultural, legal, age, or other personal reasons – would not move around the entire perimeter of a given place. That would make the scenario near real-life.
...
A particularly interesting feature is the implementation of social distance in the model. That was achieved with the `in-radius` command as shown in the blocks of codes below:
